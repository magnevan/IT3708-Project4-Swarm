...
class StagnationLayer(Layer):
    ...
    def act(self, proximities, lights, acceleration, previous_layer_did_suppress):
        ...
        self._prev_observations.append((
            proximities,
            lights,
            acceleration,
        ))

        # Filter out old ones and just keep the 10 most recent
        self._prev_observations = self._prev_observations[-10:]
        ...
        if self._has_stagnated() and previous_layer_did_suppress:
            self._still_cnt += 1
        else:
            self._still_cnt = 0
    ...
    def _has_stagnated(self):
        ...
        if any(abs(max(i_sensor_obs) - min(i_sensor_obs)) > 150 for i_sensor_obs in zip(*proximity_observations)):
            return False

        # See if light has changed in recent future
        if any(abs(max(i_sensor_obs) - min(i_sensor_obs)) > 150 for i_sensor_obs in zip(*light_observations)):
            return False

        # See if acceleration has changed in recent future
        if any(abs(max(i_sensor_obs) - min(i_sensor_obs)) > 0.1 for i_sensor_obs in zip(*acceleration_observations)):
            return False
